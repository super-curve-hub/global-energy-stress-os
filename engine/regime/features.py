...
from engine.utils import bounded_series, ewma_zscore


def build_market_features(
    px: pd.DataFrame, z_span: int = 60, z_clip: float = 3.0
) -> pd.DataFrame:
    ...
    rv10 = ret.get("WTI", pd.Series(index=px.index, dtype=float)).rolling(
        10
    ).std() * np.sqrt(252)
    rv60 = ret.get("WTI", pd.Series(index=px.index, dtype=float)).rolling(
        60
    ).std() * np.sqrt(252)
    ...
    try:
        ...
        if len(reg) > 80 and len(macro_cols) >= 2:
            ...
            resid.loc[reg.index] = reg["WTI"].values - model.predict(
                reg[macro_cols].values
            )
    ...
    if "XLE" in ret.columns and "SPX" in ret.columns:
        df["Energy_Relative_z"] = ewma_zscore(
            ret["XLE"].rolling(20).sum() - ret["SPX"].rolling(20).sum(), z_span, z_clip
        )
    else:
        df["Energy_Relative_z"] = 0.0
    if "TANKER_PROXY" in ret.columns:
        df["Tanker_Stress_z"] = ewma_zscore(
            ret["TANKER_PROXY"].rolling(5).sum(), z_span, z_clip
        )
    else:
        df["Tanker_Stress_z"] = 0.0
    df["Dollar_Stress_z"] = ewma_zscore(
        ret.get("DXY", pd.Series(index=px.index, dtype=float)).rolling(5).sum(),
        z_span,
        z_clip,
    )
...
def add_news_features(
    df: pd.DataFrame,
    gdelt_index: pd.DataFrame,
    classified_articles: pd.DataFrame,
    z_span: int = 60,
    z_clip: float = 3.0,
) -> pd.DataFrame:
    out = df.copy()
